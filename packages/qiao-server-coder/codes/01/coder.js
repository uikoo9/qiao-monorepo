...
function genController(destFolder, data) {
  ...
  var controllerDest = path.resolve(
    destFolder,
    './server/manage-db/' + data.tableName1 + '/controller/' + data.className1 + 'Controller.js',
  );
...
function genService(destFolder, data) {
  ...
  var serviceDest = path.resolve(
    destFolder,
    './server/manage-db/' + data.tableName1 + '/service/' + data.className1 + 'Service.js',
  );
...
function genModel(destFolder, data) {
  ...
  var modelDest = path.resolve(
    destFolder,
    './server/manage-db/' + data.tableName1 + '/model/' + data.className1 + 'Model.js',
  );
...
function genSql(destFolder, data) {
  ...
  var sqlDest = path.resolve(
    destFolder,
    './server/manage-db/' + data.tableName1 + '/model/' + data.tableName1 + '-' + data.tableName2 + '-sql.json',
  );
...
function genHtml(destFolder, data) {
  ...
  var htmlDest = path.resolve(
    destFolder,
    './webroot-dev/views/manage/' + data.tableName1 + '/' + data.tableName1 + '-' + data.tableName2 + '-edit.html',
  );
