...
// cmd for init code
qiao.cli.cmd.command('init <code> <path>').alias('i').description('init code to path').action(initCode);
...
async function initCode(code, path) {
  ...
  if (qiao.config.codes.indexOf(code) == -1) {
    console.log(
      'error code, see: https://github.com/uikoo9/qiao-monorepo/tree/master/packages/qiao-server-coder#code-list',
    );
    return;
  }
